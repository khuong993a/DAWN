...
const fetchPoints = async (headers, agent) => {
    try {
        const response = await axios.get(apiEndpoints.getPoints, { headers, httpsAgent: ignoreSslAgent, httpAgent: agent });
...
const keepAliveRequest = async (headers, email, agent) => {
    const payload = {
        username: email,
    ...
    try {
        const response = await axios.post(apiEndpoints.keepalive, payload, { headers, httpsAgent: ignoreSslAgent, httpAgent: agent });
...
const processAccount = async (account, proxy) => {
    ...
    // Cấu hình agent SOCKS5
    const agent = proxy ? new SocksProxyAgent(`socks5://${proxy}`) : undefined;

    const points = await fetchPoints(headers, agent);
    ...
    const success = await keepAliveRequest(headers, email, agent);
    if (success) {
